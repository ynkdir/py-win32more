...
if FAILED(CoCreateInstance(byref(ShellLink), None, CLSCTX_CLSCTX_INPROC_SERVER, byref(IShellLinkW.Guid), shell_link)):
    raise RuntimeException("CoCreateInstance")

persist_file = IPersistFile()
if FAILED(shell_link.QueryInterface(byref(IPersistFile.Guid), persist_file)):
