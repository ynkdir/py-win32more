...
py -m win32generator --loglevel=DEBUG --output-directory=%WATCHDIR% json\Windows.Win32.json.xz json\WindowsSDK.json.xz json\WindowsAppSDK.json.xz || exit
...
py -m win32generator --loglevel=DEBUG --output-directory=%WATCHDIR% --package-name=win32sel --selector=%TESTDIR%\win32sel.txt json\Windows.Win32.json.xz || exit

rmdir /s /q %WATCHDIR%\winrtsel
py -m win32generator --loglevel=DEBUG --output-directory=%WATCHDIR% --package-name=winrtsel --selector=%TESTDIR%\winrtsel.txt json\WindowsSDK.json.xz json\WindowsAppSDK.json.xz || exit
